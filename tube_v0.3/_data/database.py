...
def GetTotalRecordsCount():
    global fullList
    return str(len(fullList * 1000) + random.randint(0, 1000))  
...
#
def From_GUI_EditVideo(_fileName, _videoTitle, _videoTags, _discription, _categories):
    ...
    metaInformationObject.videoTags = re.split("\s", _videoTags)
    metaInformationObject.discription = _discription
    metaInformationObject.categories = re.split("\s", _categories)
    SaveMetaInformationObjectList()
    return "<script>alert(\"Your video got successfully updated.\");location.href = \"webgui\";</script>"
    
def From_GUI_AddVideo(_fileName, _videoTitle, _videoTags, _discription, _categories):  
    metaInformationObject = modules.MetaInformationObject()
    ...
    thumbnailPath = "/static/storage/specials/thumbnails/" + folderName + "/0.png"
    metaInformationObject.thumbnailPath = thumbnailPath
    print("DEBUG: " + _fileName)
    CreateThumbnails(_fileName)
    if MetaInformationObejctDuplication(metaInformationObject.videoUrl) == False:
        global metaInformationObjectList
        metaInformationObjectList.append(metaInformationObject)
        SaveMetaInformationObjectList()
        return "<script>alert(\"Your video got successfully added to the database.\");location.href = \"webgui\";</script>"  
    return "invalid"
    
def From_GUI_RemoveVideo(_id):
    metaInformationObject = GetMetaInformationObjectById(_id)
    if metaInformationObject == "invalid":
        return render_template("error.hmlt", error=modules.ErrorMessage("Video not Found in Database", "Check the id you enterd"))
