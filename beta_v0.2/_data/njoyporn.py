...
from nJoyPorn import database, analytics, api, communication
...
def videoPlayer():
    ...
    if "special" in url:
        ...
        if database.userDatabase.CheckCredentials(username, password) == True:
            ...
            except:#because i have old vids in db and i am to lazy to update during testing
                videoOwnerNickName = "nJoyPorn"
            print("metainformationObject.price: ", metaInformationObject.price)
            return render_template("videoPlayer.html", metaInformationObject=metaInformationObject, isLogedIn = True, userId = userObject.id, videoOwnerNickName=videoOwnerNickName)
        metaInformationObject = database.CreateDummyObject()
        return render_template("videoPlayer.html", metaInformationObject=metaInformationObject, isLogedIn = False, videoOwnerNickName="nJoyPorn")
    metaInformationObject = database.GetMetaInfromationObjectByVideoUrl(url)
    analytics.AnalyticalObjectExits(url)
    if metaInformationObject == "invalid":
        metaInformationObject = database.CreateDummyObject()
...
#https://www.njoyporn.com/video_01552022_dirty-talking-cumsluts
@app.route("/video_01552022_dirty-talking-cumsluts")
def preview():
    metaInformationObject = database.GetMetaInformationObjectById("bee546c7bc5bc577b2d186b19e2b7068")
    analytics.AnalyticalObjectExits(metaInformationObject.videoUrl)
    return render_template("videoPlayer.html", metaInformationObject=metaInformationObject, videoOwnerNickName="nJoyPorn") 
...
def setupUserPage():
    ...
    if database.userDatabase.CheckCredentials(username, password) == True:
        ...
        os.mkdir(os.getcwd()+"/data/userPages/"+username)
        os.mkdir(os.getcwd()+"/data/userPages/"+username+"/images/")
        for image in images:
            fileName = database.GetFileName(str(image))
            image.save(os.getcwd()+"/data/userPages/"+username+"/images/"+fileName)
...
def userPage():
    ...
    if database.userDatabase.CheckCredentials(username, password) == True:
        ...
        return render_template("userPage.html", userPageObject=userPageObject, isLogedIn = True)
    return render_template("userPage.html", userPageObject=userPageObject, isLogedIn = False)
...
#             userId = request.form["id"]
#             if command == "forSale":
#                 if database.userDatabase.UserCanConnectSellVideoToTrailer(trailerId, userId) == False:
#                     print("is for sale not allowd not the owner ", userId, "trailer-id:",trailerId)
#                     return render_template("error.html", error = database.modules.ErrorMessage("Permission denied", "You are not allowed to be here")) 
#             if command == "forSale" and trailerId == "":
#                 "<script>alert(\"You need to enter a trailer video id\");location.href = \"/userAccount\";</script>"
#             print("Saving File to storage: >>", file)
#             fileName = database.GetFileName(str(file))
#             file.save(database.os.getcwd()+ "/" + database.videoStoragePath + fileName)
#             return database.From_GUI_AddUserVideo(fileName, videoTitle, videoTags, discription, categories, username, trailerId, command, price)
#     return render_template("error.html", error = database.modules.ErrorMessage("Permission denied", "You are not allowed to be here"))  
...
def support():
    ...
    username, password = GetCookieData(cookieValue)
    if database.userDatabase.CheckCredentials(username, password):
        receiverId = "09a3401a97098f7257a81c8502236340"
        userObject = database.userDatabase.GetUserByCredentials(username, password)
...
def addToCard():
    ...
    if database.userDatabase.CheckCredentials(username, password):
        videoId = request.form["id"]
        userObject = database.userDatabase.AddItemToCard(videoId, username, password)
        #userObject = database.userDatabase.GetUserByCredentials(username, password)
        cardList = database.From_GUI_GetObjectsInUserCardList(userObject.cardList)
        if len(cardList) < 1:
            mio = database.modules.MetaInformationObject()
...
def buyVideo():
    ...
    if database.userDatabase.CheckCredentials(username, password):
        ...
        file = open(os.getcwd()+"/data/__/PurchaseRequests", "a")
...
def followUser():
    ...
    if database.userDatabase.CheckCredentials(username, password) == True:      
        ...
        return "<script>alert(\"Your now following:" + database.userDatabase.GetUserById(userToFollowId).nickName + ".\");location.href = \""+userToFollowId+"\";</script>"
...
# @app.route("/buyVideo_", methods=["post"])
# def buyVideo_():
#     cookieValue = request.cookies.get("data")
#     username, password = GetCookieData(cookieValue)
#     if database.userDatabase.CheckCredentials(username, password):
#         videoId = request.form["id"]
#         userId = database.userDatabase.GetUserByCredentials(username, password).id
#         file = open(os.getcwd()+"/static/data/__/dummyPurchaseRequests", "a")
#         line = "User-Id: " + userId + " Video-Id: " + videoId
#         file.write(line+"\n")
#         file.close()
#         return redirect(api.BtcPayment(userId, videoId)['url'])

@app.route("/payVideoPlayer", methods=["post"])
def payVideoPlayer():
    cookieValue = request.cookies.get("data")
    username, password = GetCookieData(cookieValue)
    if database.userDatabase.CheckCredentials(username, password) == True:
        userObject = database.userDatabase.GetUserByCredentials(username, password)
        id = request.form["id"]
        if database.userDatabase.UserHasPurchasedVideo(userObject.id, "9b96d3d3b512c98f1247ee868ee1de43"):
            metaInformationObject = database.GetMetaInformationObjectById("9b96d3d3b512c98f1247ee868ee1de43")
            videoOwnerNickName = database.userDatabase.LoadUserObjectById(metaInformationObject.owner).nickName
            return render_template("payVideoPlayer.html", metaInformationObject=metaInformationObject, isLogedIn = True, userId=userObject.id, videoOwnerNickName=videoOwnerNickName)
    return "error not allowed"
@app.route("/<path:_userId>")
def directPage(_userId):
    cookieValue = request.cookies.get("data")
    username, password = GetCookieData(cookieValue)
    userPageObject = database.userDatabase.GetUserPageDataByUserId(_userId)
    if userPageObject == "invalid":
        userPageObject = database.userDatabase.GetUserPageDataByUserNickName(_userId)
    if userPageObject == "invalid":
        return "file not found"
    userPageObject = database.userDatabase.AddMetaInformationObjectsToUserPageObjectVideoList(userPageObject)
    if database.userDatabase.CheckCredentials(username, password) == True:
...
if __name__ == "__main__":
    ...
    #os.system('python api.py -listenLoop')
