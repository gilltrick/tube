import os, re, datetime, hashlib, database, modules, pickle
from flask import render_template
from pathlib import Path

from njoyporn import userPage
...
def SetupUserPage(_nickName, _username, _discription, _catchPhrase, _categories):
    ...
    imagePathList = []
    imageFileList = os.listdir(os.getcwd()+"/static/userPages/"+_username+"/images/")
    if len(imageFileList) > 0:
        for image in imageFileList:
            imagePathList.append("/static/userPages/"+_username+"/images/"+image)
