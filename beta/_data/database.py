import os, re, cv2, pickle, datetime, hashlib, random, requests, pornhubScraper, modules, fapsterScraper, userDatabase, analytics, hotexporntoonsScraper, api
...
def LoadUserPageCategorieList():
    file = open(os.getcwd()+"/static/data/dictionarys/userPageCategorieDictionary")
...
def AddForSaleIdToMetaInformationObject(_trailerId, _videoId):
...
def From_GUI_AddVideo(_fileName, _videoTitle, _videoTags, _discription, _categories, _username, _trailerId, _command, _price):  
    ...
    if MetaInformationObejctDuplication(metaInformationObject.videoUrl) == False:
        ...
        if _trailerId != "" and _command == "forSale":
            AddForSaleIdToMetaInformationObject(_trailerId, metaInformationObject.id)
