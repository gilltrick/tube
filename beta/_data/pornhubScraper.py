import requests, re, os, pickle, hashlib, datetime, modules
...
def InitDB():
    if os.path.exists(os.getcwd()+"/static/data/databases/pornhubdb.db"):
        LoadMetaInformationObjectList()
        return
    print("PORNHUB: No database file found")
    command = input("PORNHUB: Create a new file? ")
    if command == "yes":
        file = open(os.getcwd()+"/static/data/databases/pornhubdb.db", "wb")
        file.close()
        Run()
...
def LoadMetaInformationObjectList():
    if os.path.exists(os.getcwd()+"/static/data/databases/pornhubdb.db"):
        global metaInformationObjectList
        file = open(os.getcwd()+"/static/data/databases/pornhubdb.db", "rb")
        try:
