import requests, pickle, os, database, userDatabase, time, re, sys, communication, modules, datetime
...
def CreateClient():
    if os.path.exists(os.getcwd()+"/api/pair.pair"):
        print("client exists")
        return 
    client = BTCPayClient.create_client(host='http://your-btc-sercer.com', code="YOURCODE")
    SaveClient(client)
...
def LoopCheck():
    ...
    while run:
        ...
        print("API: templist >> " + str(len(tempPaidList)) + " tempPaidList >> " + str(len(fullList)) +" paidAndDeliverdList >> "+ str(len(paidAndDeliverdList)))
        match = False
